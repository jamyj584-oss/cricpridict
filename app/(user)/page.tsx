...
import { collection, onSnapshot, query, orderBy, getDocs, where, limit, doc } from "firebase/firestore";
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    // Find the first upcoming match to show its AI pick
    const firstUpcoming = matches.find(m => m.status === 'Upcoming') || matches[0];
    if (!firstUpcoming) return;

    setAiLoading(true);
    const unsubAi = onSnapshot(doc(db, "ai_picks", firstUpcoming.id), (docSnap) => {
        if (docSnap.exists()) {
            setAiResult(docSnap.data().text || "");
            setAiError("");
        } else {
            setAiResult("");
        }
    ...
    <main className="min-h-screen bg-[#0F1115] text-white">
      ...
      <div className="grid grid-cols-4 gap-3 px-4 mb-8">
          {[
              ...
              { icon: User, label: "Profile", color: "text-accent", link: "/profile" },
              { icon: Wallet, label: "Wallet", color: "text-white", link: "/wallet" }
          ].map((item, idx) => (
      ...
      {/* AI Smart Pick */}
      <div className="p-4">
          <div className="bg-gradient-to-r from-[#161B22] to-[#1A2234] border border-white/5 rounded-2xl p-4 flex flex-col gap-4 group overflow-hidden relative">
              <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1">
                      ...
                      <p className="text-[10px] text-textMuted mt-0.5">Real-time AI squad predictions based on pitch reports & form.</p>
                  </div>
              </div>
      ...
      {/* Disclaimer */}
      <div className="text-center px-8 mt-4 mb-20 text-[9px] font-bold text-textMuted uppercase tracking-widest opacity-60 leading-relaxed">
