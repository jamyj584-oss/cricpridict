                 ...
                 <div className="bg-[#161B22] p-8 rounded-[2rem] border border-white/5 mb-8 shadow-2xl relative">
                    ...
                    {viewMode === "TEAMS" ? (
                        <form onSubmit={handleSaveTeam} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div>
                                <label className="text-[10px] text-textMuted uppercase tracking-widest font-black mb-2 block flex items-center gap-2"><ImageIcon size={12} /> Team Logo URL</label>
                                <input type="text" value={teamLogo} onChange={e=>setTeamLogo(e.target.value)} className="w-full bg-[#0F1115] border border-white/5 rounded-2xl px-5 py-4 text-sm focus:border-accent/50 outline-none transition-colors" placeholder="Link to team image/logo" />
                            </div>
